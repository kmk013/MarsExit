...
//			GameManager.gm.mainPlayer.transform.LookAt (transform);
		...
		if (hp <= 0) {
			...
			drop = true;
			if (drop && dropInt < 1) {
				Instantiate (GameManager.gm.meat3DP, this.gameObject.transform.position, Quaternion.identity);
				Instantiate (GameManager.gm.water3DP, this.gameObject.transform.position, Quaternion.identity);
				dropInt++;
			}
			drop = false;
			Invoke ("destory", 4.6f);
